...
type StyleCyto = {selector: string, style: {[key: string]: any}};
...
const highlighted: string = chroma('rgb(62, 189, 147)').alpha(0.5).hex('rgb');
const primary = colors[0];
const secondary = colors[1];

const style: StyleCyto[] = [{
    'selector': 'core',
    'style': {
 ...
 * @returns 
 */
const buildColorScale = (importedOntologies: any[], ontologyId: string): {style: StyleCyto[], ontologyColorMap: {[key:string]: string}} => {
    const tempStyles = []
...
const getScale = (no: number, isPaired?: boolean): string[] => {
    if (isPaired) {
        return
    }
