...
import { forkJoin, from, merge as rxjsMerge, Observable, of, Subject, throwError } from 'rxjs';
import { v4 as uuidv4 } from 'uuid';

import { CATALOG, DCTERMS, ONTOLOGYEDITOR, ONTOLOGYSTATE, OWL, RDF, RDFS, SKOS, XSD } from '../../prefixes';
...
export class OntologyStateService extends VersionedRdfState<OntologyListItem> {
  ...
  /**
   * Removes the user's In Progress Commit for the currently selected OntologyRecord. Calls the Catalog endpoint,
      ...
      .pipe(
        ...
        catchError((response: HttpErrorResponse): Observable<string> => {
          if (typeof response === 'string') {
            ...
            return throwError({
              errorMessage: 'No changes were found in the uploaded file.',
              errorDetails: []
            });
          } else {
            return throwError({
  ...
  createState(versionedRdfStateBase: VersionedRdfStateBase): Observable<null> {
    return super.createState(versionedRdfStateBase).pipe(finalize(() => {
      this._ontologyRecordActionSubject.next({
        recordId: versionedRdfStateBase.recordId,
  ...
  getGroupedSelectList(list: string[], searchText: string, getName = this.getEntityName):
    ...
    const grouped: {
      [key: string]: { item: string, name: string }[]
    } = groupBy(array, item => getIRINamespace(item.item));
