...
export class MapperStateService {
  ...
  /**
   * Sets the state variables to indicate creating a new mapping.
   */
  ...
  setMasterBranch(): Observable<null> {
    const catalogId = get(this.cm.localCatalog, '@id', '');
    return this.cm.getRecordBranch('master', this.selected.record.id, catalogId)
   ...
   * @param {boolean} [isId=false] True if it has an '@id'
   */
   ...
   * @param {boolean} [isTracked=false] A boolean indicating if the ontology to query against is tracked.
   ...
   * @param {string} searchText Optional text to filter the list of properties by. Searches over the calculated names of
   *                            the properties. Can be an empty string.
   * @param {number} [limit=100] An optional limit to apply to the list of results. Defaults to 100.
   * @param {boolean} [isTracked=false] A boolean indicating if the ontology to query against is tracked.
   ...
   * 
   * @param {MappingOntologyInfo} ontInfo The metadata about which ontology to search within.
