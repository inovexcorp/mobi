...
export class UtilService {
     ...
     * @return {Promise} A Promise that rejects with an error message
     */
    rejectErrorObject(error: HttpErrorResponse, defaultMessage = ''): Promise<RESTError> {
        return Promise.reject(get(error, 'status') === -1 ? {error: '', errorMessage: '', errorDetails: []} : this.getErrorDataObject(error, defaultMessage));
    }
    /**
     * Retrieves an error message from a HTTP response if available, otherwise uses the passed default
     * message.
     ...
     * @return {string} An error message for the passed HTTP response
     */
    getErrorDataObject(error: HttpErrorResponse, defaultMessage = 'Something went wrong. Please try again later.'): RESTError {
        const statusText = get(error, 'statusText');
        return {
     ...
     * @returns 
     */
    handleErrorObject(error: HttpErrorResponse): Observable<RESTError> {
        if (error.status === 0) {
