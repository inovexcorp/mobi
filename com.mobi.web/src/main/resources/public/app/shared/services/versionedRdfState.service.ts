    ...
    getMergeDifferences(sourceCommitId: string, targetCommitId: string, limit: number, offset: number): Observable<null> {
        ...
        return this.cm.getDifference(sourceCommitId, targetCommitId, limit, offset)
            .pipe(map((response: HttpResponse<CommitDifference>) => {
                ...
                this.listItem.merge.difference.additions = concat(this.listItem.merge.difference.additions as JSONLDObject[], response.body.additions as JSONLDObject[]);
                this.listItem.merge.difference.deletions = concat(this.listItem.merge.difference.deletions as JSONLDObject[], response.body.deletions as JSONLDObject[]);
