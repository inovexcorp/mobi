...
describe('Property Values component', function () {
  ...
  it('should call edit when the appropriate button is clicked', function () {
    ...
    spyOn(component, 'callEdit');
    const editBtn = element.query(By.css('[title="Edit"]'));
    expect(editBtn).toBeTruthy();

    editBtn.triggerEventHandler('click', null);
    expect(component.callEdit).toHaveBeenCalledWith(0);
  });
  it('should call remove when the appropriate button is clicked', () => {
    ...
    const removeButton = element.query(By.css('.button-container [title="Delete"]'));
    expect(removeButton).toBeTruthy();

    removeButton.triggerEventHandler('click', null);
    expect(component.callRemove).toHaveBeenCalledWith(0);
  });
});
