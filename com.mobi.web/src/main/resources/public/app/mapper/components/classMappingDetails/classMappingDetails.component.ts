...
import { getBeautifulIRI, getEntityName, getPropertyId, getPropertyValue } from '../../../shared/utility';
...
/**
 * @class ClassMappingDetailsComponent
 *
 * A component that creates a div with sections to view and edit information about a Class Mapping identified by the
 * provided ID. One section is for viewing and editing the {@link IriTemplateOverlayComponent} IRI template of the
 * class mapping. Another section is for viewing the list of property mappings associated with the class mapping,
 * adding to that list, editing items in the list, and removing from that list. The component houses methods for
 * opening the modals for {@link PropMappingOverlayComponent} editing, adding, and removing PropertyMappings.
 */
@Component({
...
export class ClassMappingDetailsComponent {
  ...
  getPropValue(propMapping: JSONLDObject): string {
    if (this.mm.isDataMapping(propMapping)) {
      ...
      return getEntityName(classMapping);
    }
  }
  ...
  confirmDeleteProp(propMapping: PropMappingPreview): void {
    ...
    this.dialog.open(ConfirmModalComponent, {
      data: {
        content: `<p>Are you sure you want to delete <strong>${propMapping.title}</strong> from <strong>${getEntityName(classMapping)}</strong>?</p>`
      }
    ...
    if (this.state.selected) {
      this.propMappings = this.state.selected.mapping.getPropMappingsByClass(this.classMappingId).map(propMapping => {
        const propMappingPreview: PropMappingPreview = {
          ...
          title: getEntityName(propMapping)
        };
        if (this.mm.isDataMapping(propMapping)) {
