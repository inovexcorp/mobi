...
(function() {
    ...
    const importsBlockComponent = {
        templateUrl: 'ontology-editor/components/importsBlock/importsBlock.component.html',
        bindings: {
            listItem: '<'
        },
        controllerAs: 'dvm',
    ...
    function importsBlockComponentCtrl($q, $timeout, ontologyStateService, prefixes, utilService, propertyManagerService, modalService) {
        ...
        dvm.remove = function() {
            ...
            pm.remove(dvm.listItem.selected, importsIRI, _.findIndex(dvm.listItem.selected[importsIRI], {'@id': dvm.url}));
        ...
        dvm.failed = function(iri) {
            return _.includes(dvm.listItem.failedImports, iri);
        }
        ...
        dvm.setIndirectImports = function() {
            var directImports = _.map(_.get(dvm.listItem.selected, prefixes.owl + 'imports'), '@id');
            var goodImports = _.map(dvm.listItem.importedOntologies, item => _.pick(item, 'id', 'ontologyId'));
            var failedImports = _.map(dvm.listItem.failedImports, iri => ({ id: iri, ontologyId: iri }));
