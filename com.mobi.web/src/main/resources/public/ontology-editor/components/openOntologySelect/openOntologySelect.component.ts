...
function openOntologySelectComponentCtrl($q, $timeout, catalogManagerService: CatalogManagerService, ontologyStateService, prefixes, ontologyManagerService, utilService, modalService) {
    ...
    dvm.$doCheck = function() {
        var recordState = find(get(dvm.state, 'model', []), {'@type': [prefixes.ontologyState + 'StateRecord']});
        ...
        if (dvm.currentStateId !== currentValue) {
            ...
            dvm.currentState = find(dvm.state.model, {'@id': dvm.currentStateId});
            setSelected();
            setSelectList();
        }
    }
    ...
    function setSelected() {
        if (dvm.os.isStateBranch(dvm.currentState)) {
            dvm.selected = find(dvm.listItem.branches, {'@id': dvm.util.getPropertyId(dvm.currentState, prefixes.ontologyState + 'branch')});
        } else if (dvm.os.isStateTag(dvm.currentState)) {
            dvm.selected = find(dvm.listItem.tags, {'@id': dvm.util.getPropertyId(dvm.currentState, prefixes.ontologyState + 'tag')});
        } else {
    ...
    function setSelectList() {
        if (dvm.cm.isBranch(dvm.selected) || dvm.cm.isTag(dvm.selected)) {
            dvm.selectList = concat(dvm.listItem.branches, dvm.listItem.tags);
        } else if (dvm.cm.isCommit(dvm.selected)) {
            dvm.selectList = concat(dvm.listItem.branches, [dvm.selected], dvm.listItem.tags);
        } else {
            dvm.selectList = [];
        }
    }
...
export default openOntologySelectComponent;
