            ...
            describe('if the entity is a branch', function() {
                ...
                describe('when getRecordBranch resolves', function() {
                    beforeEach(function() {
                        ...
                        catalogManagerSvc.getRecordBranch.and.returnValue($q.when({ 'http://mobi.com/ontologies/catalog#head': [ {'@id': this.commitId} ] }));
                ...
                it('when getRecordBranch rejects', function() {
                    expect(this.controller.deleteError).toEqual('');
                    catalogManagerSvc.getRecordBranch.and.returnValue($q.reject(this.errorMessage));
                    this.controller.changeEntity(this.branch);
                    scope.$digest();
                    expect(catalogManagerSvc.getRecordBranch).toHaveBeenCalledWith(this.branchId, this.recordId, this.catalogId);
        ...
        describe('openDeleteConfirmation calls the correct methods if the entity is', function() {
            ...
            describe('a branch and', function() {
                ...
                it('a user branch', function() {
                    ...
                    expect(modalSvc.openConfirmModal).toHaveBeenCalledWith(jasmine.stringMatching('diverging changes'), jasmine.any(Function));
                });
            ...
            describe('when deleteOntologyBranch is resolved', function() {
                ...
                describe('and when removeBranch is resolved', function() {
                    ...
                    it('and when deleteOntologyBranchState is rejected', function() {
                        ontologyStateSvc.deleteOntologyBranchState.and.returnValue($q.reject(this.errorMessage));
                        this.controller.deleteBranch(this.branch);
                        scope.$apply();
                        expect(ontologyManagerSvc.deleteOntologyBranch).toHaveBeenCalledWith(this.recordId, this.branchId);
                        expect(ontologyStateSvc.removeBranch).toHaveBeenCalledWith(this.recordId, this.branchId);
