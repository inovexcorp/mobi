 ...
 * #L%
 */
import { map, includes, filter, without } from 'lodash';

const template = require('./relationshipOverlay.component.html');
...
function relationshipOverlayComponentCtrl(ontologyManagerService, ontologyStateService, utilService, ontologyUtilsManagerService, propertyManagerService) {
    ...
    dvm.$onInit = function() {
        dvm.conceptList = without(om.getConceptIRIs(dvm.os.getOntologiesArray(), dvm.os.listItem.derivedConcepts), dvm.os.listItem.selected['@id']);
        dvm.schemeList = without(om.getConceptSchemeIRIs(dvm.os.getOntologiesArray(), dvm.os.listItem.derivedConceptSchemes), dvm.os.listItem.selected['@id']);
    }
    dvm.addRelationship = function() {
        var addedValues = filter(dvm.values, value => pm.addId(dvm.os.listItem.selected, dvm.relationship, value));
        if (addedValues.length !== dvm.values.length) {
            dvm.util.createWarningToast('Duplicate property values not allowed');
        }
        if (addedValues.length) {
            var addedValueObjs = map(addedValues, value => ({'@id': value}));
    ...
    dvm.getValues = function(searchText) {
        var isSchemeRelationship = includes(pm.conceptSchemeRelationshipList, dvm.relationship);
        var isSemanticRelation = includes(dvm.os.listItem.derivedSemanticRelations, dvm.relationship);
        var list = [];
...
export default relationshipOverlayComponent;