...
const template = require('./datasetsOntologyPicker.component.html');
...
function datasetsOntologyPickerComponentCtrl(httpService, catalogManagerService, utilService, prefixes) {
    ...
    function parseOntologyResults(response) {
        ...
        dvm.ontologies = map(response.data, record => ({
            ...
            selected: some(dvm.selectedOntologies, {recordId: record['@id']}),
            jsonld: record
        }));
    }
}

export default datasetsOntologyPickerComponent;
