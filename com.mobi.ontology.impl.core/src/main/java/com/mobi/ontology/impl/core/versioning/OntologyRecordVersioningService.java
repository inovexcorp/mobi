...
public class OntologyRecordVersioningService extends BaseVersioningService<OntologyRecord> {
    ...
    protected void updateMasterRecordIRI(VersionedRDFRecord record, Commit commit, RepositoryConnection conn) {
        ...
        OntologyRecord ontologyRecord = ontologyRecordFactory.getExisting(record.getResource(), record.getModel())
                .orElseThrow(() -> new IllegalStateException("Record expected to be of type OntologyRecord"));
        Resource existingOntologyIRI = record.getTrackedIdentifier()
                .orElseThrow(() -> new IllegalStateException("OntologyRecord " + record.getResource()
