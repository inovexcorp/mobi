package org.matonto.ontology.core.impl.owlapi;

import org.semanticweb.owlapi.model.AxiomType;

public enum SimpleAxiomType {

	DECLARATION ("Declaration", true, true, false),	
	EQUIVALENT_CLASSES ("EquivalentClasses", false, false, true), 
	SUBCLASS_OF ("SubClassOf", false, false, true), 
	DISJOINT_CLASSES ("DisjointClasses", false, false, true), 
	DISJOINT_UNION ("DisjointUnion", true, false, true),
	CLASS_ASSERTION ("ClassAssertion", false, false, true), 
	SAME_INDIVIDUAL ("SameIndividual", false, false, true), 
	DIFFERENT_INDIVIDUALS ("DifferentIndividuals", false, false, true), 
	OBJECT_PROPERTY_ASSERTION ("ObjectPropertyAssertion", false, false, true), 
	NEGATIVE_OBJECT_PROPERTY_ASSERTION ("NegativeObjectPropertyAssertion", true, false, true), 
	DATA_PROPERTY_ASSERTION ("DataPropertyAssertion", false, false, true), 
	NEGATIVE_DATA_PROPERTY_ASSERTION ("NegativeDataPropertyAssertion", true, false, true),
	EQUIVALENT_OBJECT_PROPERTIES ("EquivalentObjectProperties", false, false, true), 
	SUB_OBJECT_PROPERTY ("SubObjectPropertyOf", false, false, true), 
	INVERSE_OBJECT_PROPERTIES ("InverseObjectProperties", false, false, true), 
	FUNCTIONAL_OBJECT_PROPERTY ("FunctionalObjectProperty", false, false, true), 
	INVERSE_FUNCTIONAL_OBJECT_PROPERTY ("InverseFunctionalObjectProperty", false, false, true), 
	SYMMETRIC_OBJECT_PROPERTY ("SymmetricObjectProperty", false, false, true), 
	ASYMMETRIC_OBJECT_PROPERTY ("AsymmetricObjectProperty", true, true, true), 
	TRANSITIVE_OBJECT_PROPERTY ("TransitiveObjectProperty", false, false, true), 
	REFLEXIVE_OBJECT_PROPERTY ("ReflexiveObjectProperty", true, true, true), 
	IRREFLEXIVE_OBJECT_PROPERTY ("IrrefexiveObjectProperty", true, true, true), 
	OBJECT_PROPERTY_DOMAIN ("ObjectPropertyDomain", false, false, true), 
	OBJECT_PROPERTY_RANGE ("ObjectPropertyRange", false, false, true), 
	DISJOINT_OBJECT_PROPERTIES ("DisjointObjectProperties", true, true, true), 
	SUB_PROPERTY_CHAIN_OF ("SubPropertyChainOf", true, true, true),
	EQUIVALENT_DATA_PROPERTIES ("EquivalentDataProperties", false, false, true), 
	SUB_DATA_PROPERTY ("SubDataPropertyOf", false, false, true), 
	FUNCTIONAL_DATA_PROPERTY ("FunctionalDataProperty", false, false, true), 
	DATA_PROPERTY_DOMAIN ("DataPropertyDomain", false, false, true), 
	DATA_PROPERTY_RANGE ("DataPropertyRange", false, false, true), 
	DISJOINT_DATA_PROPERTIES ("DisjointDataProperties", true, true, true),
	DATATYPE_DEFINITION ("DatatypeDefinition", true, true, true),
	HAS_KEY ("HasKey", true, true, true),
	SWRL_RULE ("Rule", false, false, true),
	ANNOTATION_ASSERTION ("AnnotationAssertion", false, false, false), 
	SUB_ANNOTATION_PROPERTY_OF ("SubAnnotationPropertyOf", true, true, false), 
	ANNOTATION_PROPERTY_RANGE ("AnnotationPropertyRangeOf", true, true, false), 
	ANNOTATION_PROPERTY_DOMAIN ("AnnotationPropertyDomain", true, true, false);
	
	
	private final String name;
	private final boolean owl2Axiom;
	private final boolean nonSyntacticOWL2Axiom;
	private final boolean isLogical;
   
	private SimpleAxiomType(String name, boolean owl2Axiom, boolean nonSyntacticOWL2Axiom, boolean isLogical)
	{
		this.name = name;
		this.owl2Axiom = owl2Axiom;
		this.nonSyntacticOWL2Axiom = nonSyntacticOWL2Axiom;
		this.isLogical = isLogical;
	}
	
	@Override
	public String toString()
	{
		return name;
	}
	
	public String getName()
	{
		return name;
	}
	
	public boolean isOWL2Axiom()
	{
		return owl2Axiom;
	}
	
	public boolean isNonSyntacticOWL2Axiom()
	{
	    return nonSyntacticOWL2Axiom;
	}
	
	public boolean isLogical()
	{
		return isLogical;
	}
	
	
	public static SimpleAxiomType matontAxiomType(AxiomType owlapiAxiomType)
	{
		SimpleAxiomType simpleAxiomType = null;
		
		switch(owlapiAxiomType.getName())
		{
			case "Declaration":
				return SimpleAxiomType.DECLARATION;
				
			case "EquivalentClasses":
				return SimpleAxiomType.EQUIVALENT_CLASSES;
				
			case "SubClassOf":
				return SimpleAxiomType.SUBCLASS_OF;
				
			case "DisjointClasses":
				return SimpleAxiomType.DISJOINT_CLASSES;
				
			case "DisjointUnion":
				return SimpleAxiomType.DISJOINT_UNION;
				
			case "ClassAssertion":
				return SimpleAxiomType.CLASS_ASSERTION;
				
			case "SameIndividual":
				return SimpleAxiomType.SAME_INDIVIDUAL;
				
			case "DifferentIndividuals":
				return SimpleAxiomType.DIFFERENT_INDIVIDUALS;
				
			case "ObjectPropertyAssertion":
				return SimpleAxiomType.OBJECT_PROPERTY_ASSERTION;
				
			case "NegativeObjectPropertyAssertion":
				return SimpleAxiomType.NEGATIVE_OBJECT_PROPERTY_ASSERTION;
				
			case "DataPropertyAssertion":
				return SimpleAxiomType.DATA_PROPERTY_ASSERTION;
				
			case "NegativeDataPropertyAssertion":
				return SimpleAxiomType.NEGATIVE_DATA_PROPERTY_ASSERTION;
				
			case "EquivalentObjectProperties":
				return SimpleAxiomType.EQUIVALENT_OBJECT_PROPERTIES;
				
			case "SubObjectPropertyOf":
				return SimpleAxiomType.SUB_OBJECT_PROPERTY;
				
			case "InverseObjectProperties":
				return SimpleAxiomType.INVERSE_OBJECT_PROPERTIES;
				
			case "FunctionalObjectProperty":
				return SimpleAxiomType.FUNCTIONAL_OBJECT_PROPERTY;
				
			case "InverseFunctionalObjectProperty":
				return SimpleAxiomType.INVERSE_FUNCTIONAL_OBJECT_PROPERTY;
				
			case "SymmetricObjectProperty":
				return SimpleAxiomType.SYMMETRIC_OBJECT_PROPERTY;
				
			case "AsymmetricObjectProperty":
				return SimpleAxiomType.ASYMMETRIC_OBJECT_PROPERTY;
				
			case "TransitiveObjectProperty":
				return SimpleAxiomType.TRANSITIVE_OBJECT_PROPERTY;
				
			case "ReflexiveObjectProperty":
				return SimpleAxiomType.REFLEXIVE_OBJECT_PROPERTY;
				
			case "IrrefexiveObjectProperty":
				return SimpleAxiomType.IRREFLEXIVE_OBJECT_PROPERTY;
				
			case "ObjectPropertyDomain":
				return SimpleAxiomType.OBJECT_PROPERTY_DOMAIN;
				
			case "ObjectPropertyRange":
				return SimpleAxiomType.OBJECT_PROPERTY_RANGE;
				
			case "DisjointObjectProperties":
				return SimpleAxiomType.DISJOINT_OBJECT_PROPERTIES;
				
			case "SubPropertyChainOf":
				return SimpleAxiomType.SUB_PROPERTY_CHAIN_OF;
				
			case "EquivalentDataProperties":
				return SimpleAxiomType.EQUIVALENT_DATA_PROPERTIES;
				
			case "SubDataPropertyOf":
				return SimpleAxiomType.SUB_DATA_PROPERTY;
				
			case "FunctionalDataProperty":
				return SimpleAxiomType.FUNCTIONAL_DATA_PROPERTY;
				
			case "DataPropertyDomain":
				return SimpleAxiomType.DATA_PROPERTY_DOMAIN;
				
			case "DataPropertyRange":
				return SimpleAxiomType.DATA_PROPERTY_RANGE;
				
			case "DisjointDataProperties":
				return SimpleAxiomType.DISJOINT_DATA_PROPERTIES;
				
			case "DatatypeDefinition":
				return SimpleAxiomType.DATATYPE_DEFINITION;
				
			case "AnnotationAssertion":
				return SimpleAxiomType.ANNOTATION_ASSERTION;
				
			case "SubAnnotationPropertyOf":
				return SimpleAxiomType.SUB_ANNOTATION_PROPERTY_OF;
				
			case "AnnotationPropertyRangeOf":
				return SimpleAxiomType.ANNOTATION_PROPERTY_RANGE;
				
			case "AnnotationPropertyDomain":
				return SimpleAxiomType.ANNOTATION_PROPERTY_DOMAIN;
			
		}
		
		return simpleAxiomType;
	}
	
	public static AxiomType owlapiAxiomType(SimpleAxiomType matontoAxiomType)
	{
		AxiomType owlapiAxiomType = null;
		
		switch(matontoAxiomType.getName())
		{
			case "Declaration":
				return AxiomType.DECLARATION;
				
			case "EquivalentClasses":
				return AxiomType.EQUIVALENT_CLASSES;
				
			case "SubClassOf":
				return AxiomType.SUBCLASS_OF;
				
			case "DisjointClasses":
				return AxiomType.DISJOINT_CLASSES;
				
			case "DisjointUnion":
				return AxiomType.DISJOINT_UNION;
				
			case "ClassAssertion":
				return AxiomType.CLASS_ASSERTION;
				
			case "SameIndividual":
				return AxiomType.SAME_INDIVIDUAL;
				
			case "DifferentIndividuals":
				return AxiomType.DIFFERENT_INDIVIDUALS;
				
			case "ObjectPropertyAssertion":
				return AxiomType.OBJECT_PROPERTY_ASSERTION;
				
			case "NegativeObjectPropertyAssertion":
				return AxiomType.NEGATIVE_OBJECT_PROPERTY_ASSERTION;
				
			case "DataPropertyAssertion":
				return AxiomType.DATA_PROPERTY_ASSERTION;
				
			case "NegativeDataPropertyAssertion":
				return AxiomType.NEGATIVE_DATA_PROPERTY_ASSERTION;
				
			case "EquivalentObjectProperties":
				return AxiomType.EQUIVALENT_OBJECT_PROPERTIES;
				
			case "SubObjectPropertyOf":
				return AxiomType.SUB_OBJECT_PROPERTY;
				
			case "InverseObjectProperties":
				return AxiomType.INVERSE_OBJECT_PROPERTIES;
				
			case "FunctionalObjectProperty":
				return AxiomType.FUNCTIONAL_OBJECT_PROPERTY;
				
			case "InverseFunctionalObjectProperty":
				return AxiomType.INVERSE_FUNCTIONAL_OBJECT_PROPERTY;
				
			case "SymmetricObjectProperty":
				return AxiomType.SYMMETRIC_OBJECT_PROPERTY;
				
			case "AsymmetricObjectProperty":
				return AxiomType.ASYMMETRIC_OBJECT_PROPERTY;
				
			case "TransitiveObjectProperty":
				return AxiomType.TRANSITIVE_OBJECT_PROPERTY;
				
			case "ReflexiveObjectProperty":
				return AxiomType.REFLEXIVE_OBJECT_PROPERTY;
				
			case "IrrefexiveObjectProperty":
				return AxiomType.IRREFLEXIVE_OBJECT_PROPERTY;
				
			case "ObjectPropertyDomain":
				return AxiomType.OBJECT_PROPERTY_DOMAIN;
				
			case "ObjectPropertyRange":
				return AxiomType.OBJECT_PROPERTY_RANGE;
				
			case "DisjointObjectProperties":
				return AxiomType.DISJOINT_OBJECT_PROPERTIES;
				
			case "SubPropertyChainOf":
				return AxiomType.SUB_PROPERTY_CHAIN_OF;
				
			case "EquivalentDataProperties":
				return AxiomType.EQUIVALENT_DATA_PROPERTIES;
				
			case "SubDataPropertyOf":
				return AxiomType.SUB_DATA_PROPERTY;
				
			case "FunctionalDataProperty":
				return AxiomType.FUNCTIONAL_DATA_PROPERTY;
				
			case "DataPropertyDomain":
				return AxiomType.DATA_PROPERTY_DOMAIN;
				
			case "DataPropertyRange":
				return AxiomType.DATA_PROPERTY_RANGE;
				
			case "DisjointDataProperties":
				return AxiomType.DISJOINT_DATA_PROPERTIES;
				
			case "DatatypeDefinition":
				return AxiomType.DATATYPE_DEFINITION;
				
			case "AnnotationAssertion":
				return AxiomType.ANNOTATION_ASSERTION;
				
			case "SubAnnotationPropertyOf":
				return AxiomType.SUB_ANNOTATION_PROPERTY_OF;
				
			case "AnnotationPropertyRangeOf":
				return AxiomType.ANNOTATION_PROPERTY_RANGE;
				
			case "AnnotationPropertyDomain":
				return AxiomType.ANNOTATION_PROPERTY_DOMAIN;
			
		}
		
		return owlapiAxiomType;
	}
	
	
}
