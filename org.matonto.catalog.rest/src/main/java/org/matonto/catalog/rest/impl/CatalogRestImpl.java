            ...
            catalogManager.getCommitChain(headIRI).stream()
                    ...
                    .map(commit -> {
                        ...
                        Value creatorIRI = commit.getProperty(factory.createIRI(Activity.wasAssociatedWith_IRI))
                                .orElse(null);
                        Value date = commit.getProperty(factory.createIRI("http://www.w3.org/ns/prov#atTime"))
                                .orElse(emptyLiteral);
                        ...
                        if (creator != null) {
                            creatorObject.element("first", creator.getFirstName().stream().findFirst()
                                    .orElse(emptyLiteral).stringValue())
                                    .element("last", creator.getLastName().stream().findFirst().orElse(emptyLiteral)
                                            .stringValue())
