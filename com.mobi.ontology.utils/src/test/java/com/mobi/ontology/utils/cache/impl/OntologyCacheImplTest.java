...
public class OntologyCacheImplTest extends OrmEnabledTestCase {
    private AutoCloseable closeable;
    private ModelFactory mf;
    private ValueFactory vf;
    private MemoryRepositoryWrapper repo;
    private OntologyCacheImpl cache;
    ...
    public void setUp() throws Exception {
        ...
        when(ontMultipleImports.getImportedOntologyIRIs()).thenReturn(Set.of(import1IRI, import2IRI, import3IRI));
        ...
        when(commitManager.commitInRecord(any(), any(), any())).thenReturn(true);
        ...
        cache = spy(new OntologyCacheImpl());
        cache.configProvider = configProvider;
        cache.commitManager = commitManager;
        cache.repository = repo;
        cache.dsUtilsService = dsUtilsService;
        cache.ontologyCreationService = ontologyCreationService;
        cache.thingManager = thingManager;
        cache.recordManager = recordManager;
    ...
    public void generateKeyTest() throws Exception {
        assertEquals("test&test", cache.generateKey("test", "test"));
        assertEquals("test&null", cache.generateKey("test", null));
        assertEquals("null&null", cache.generateKey(null, null));
    }

    @Test
    public void clearCacheAndHitTest() throws Exception {
        cache.put(key2, ontOneImport);
        cache.clearCache(vf.createIRI("urn:record2"));
        verify(cache).remove(key2);
    }

    @Test
    public void clearCacheAndMissTest() throws Exception {
        cache.clearCache(vf.createIRI("http://test.com/missing"));
    ...
    public void clearCacheImportsWithHitTest() throws Exception {
        ...
        cache.clearCacheImports(import1IRI);
        verify(cache).removeAll(Collections.singleton(key3));
    }

    @Test
    public void clearCacheImportsWithMissTest() throws Exception {
        cache.clearCacheImports(vf.createIRI("http://test.com/missing"));
    ...
    public void removeFromCacheTest() throws Exception {
        cache.put(key2, ontOneImport);
        cache.removeFromCache("urn:record2", "urn:commit1");
        verify(cache).containsKey(key2);
        verify(cache).remove(key2);
    }

    @Test
    public void removeFromCacheWhenNotContained() throws Exception {
        cache.removeFromCache("urn:record1", "urn:commit1");
        verify(cache).containsKey(key1);
    ...
    public void getNameTest() {
        assertEquals("ontologyCache", cache.getName());
    }
