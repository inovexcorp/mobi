...
import com.mobi.catalog.api.record.RDFExportService;
...
@Component
public class RDFExportConfigImpl implements RDFExportService {

    private static final Logger LOGGER = LoggerFactory.getLogger(RDFExportConfigImpl.class);
    ...
    @Override
    public void export(RDFExportRecordConfig config, String repositoryID) throws IOException {
    ...
    @Override
    public void export(RDFExportRecordConfig config, Model model) throws IOException {
        export(model, config.getOutput(), config.getFormat());
    }

    private void export(RepositoryConnection conn, RDFExportRecordConfig config) throws IOException {
    ...
    private Resource getSubject(RDFExportRecordConfig config) {
        return config.getSubj() == null ? null : vf.createIRI(config.getSubj());
    }

    private IRI getPredicate(RDFExportRecordConfig config) {
        return config.getPred() == null ? null : vf.createIRI(config.getPred());
    }

    private Value getObject(RDFExportRecordConfig config) {
        Value objValue = null;
    ...
    private Resource getGraph(RDFExportRecordConfig config) {
